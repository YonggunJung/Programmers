...
namespace ProgrammersLv1
{
    internal class 신고_결과_받기
    {
        ...
        public int[] solution66(string[] id_list, string[] report, int k)
        {
            // HashSet 하나 배워감
            int[] answer = new int[id_list.Length] ;
            HashSet<string> hs = new HashSet<string>(report);
            ...
            foreach (var sre in hs)
            {
                ...
                if (dic[re[1]] >= k)
                {
                    answer[Array.IndexOf(id_list, re[0])]++;
                }
            }
            return answer;
        }
