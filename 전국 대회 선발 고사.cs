...
{
    internal class 전국_대회_선발_고사
    {
        //문제 설명
        //0번부터 n - 1번까지 n명의 학생 중 3명을 선발하는
        //전국 대회 선발 고사를 보았습니다.
        //등수가 높은 3명을 선발해야 하지만,
        ...
        public int solution102(int[] rank, bool[] attendance)
        {
            int answer = 0;
            List<int> li = new List<int>();
            List<int> rli = new List<int>(rank);
            for (int i = 0; i<rank.Length; i++)
            {
                if (attendance[i]) li.Add(rank[i]);
            }
            li.Sort();
            answer = rli.IndexOf(li[0]) * 10000 + rli.IndexOf(li[1]) * 100 + rli.IndexOf(li[2]);
            return answer;
        }
        //다른 사람 풀이
        //Array.IndexOf(rank, i) 이걸 몰랐군
        //    List<int> arr = new List<int>();
        //    for (int i = 1; i <= rank.Length; i++)
        //    {
        //        if (arr.Count == 3)
        //            break;
        //        if (attendance[Array.IndexOf(rank, i)])
        //            arr.Add(Array.IndexOf(rank, i));
        //    }
