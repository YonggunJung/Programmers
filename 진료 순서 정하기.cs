...
namespace Programmers
{
    internal class 진료_순서_정하기
    {
        ...
        //입출력 예 설명
        //입출력 예 #1
        //emergency가[3, 76, 24] 이므로 응급도의 크기 순서대로 번호를 매긴
        //[3, 1, 2] 를 return합니다.
        ...
        public int[] solution192(int[] emergency)
        {
            int[] answer = new int[emergency.Length];
            int[] e2 = (int[])emergency.Clone();
            Array.Sort(e2);
            Array.Reverse(e2);
            for (int i = 0; i < answer.Length; i++)
            {
                answer[i] = Array.IndexOf(e2, emergency[i])+1;
            }
            return answer;
        }
        //다른 사람 풀이
