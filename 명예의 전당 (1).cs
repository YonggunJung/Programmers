...
namespace ProgrammersLv1
{
    internal class 명예의_전당__1_
    {
        ...
        //출연 가수의 점수가 명예의 전당에 오르게 되고 기존의
        //k번째 순위의 점수는 명예의 전당에서 내려오게 됩니다.
        ...
        //명예의 전당 목록의 점수의 개수 k, 1일부터
        //마지막 날까지 출연한 가수들의 점수인 score가 주어졌을 때,
        ...
        public int[] solution44(int k, int[] score)
        {
            // 문제 이해를 못해서 30분 날림.
            int[] answer = new int[] { };
            ...
            for (int i  = 0; i < score.Length; i++)
            {
                ...
                {
                    li.Sort();
                    if (li[0] < score[i]) li[0] = score[i];
                    ali.Add(li.Min());
                }
            }
            //ali.Sort(); // 이게 문제였다 착각을 했다.
            answer = ali.ToArray();
            return answer;

            // 이거 왜 안될까? ㅠㅠ
            // 이 코드는 문제가 없다. 내가 문제였다.
            //for (int i = 0; i < score.Length; i++)
            //{
            //    if (i < k)
            //    {
