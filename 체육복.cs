...
namespace ProgrammersLv1
{
    internal class 체육복
    {
        ...
        public int solution58(int n, int[] lost, int[] reserve)
        {
            int answer = 0;
            int lostlen = lost.Length;
            List<int> lo = new List<int>(lost);
            List<int> re = new List<int>(reserve);
            lo.Sort();
            re.Sort();
            ...
            foreach(int num in lo)
            {
                if (re.Contains(num - 1))
                {
                    lostlen--;
                    re.Remove(num - 1);
                }
                else if (re.Contains(num + 1))
                {
                    lostlen--;
                    re.Remove(num + 1);
                }
