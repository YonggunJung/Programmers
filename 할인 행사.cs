...
namespace ProgrammersLv2
{
    class 할인_행사
    {
        ...
        //둘째 날부터 열흘 간에는 바나나를 원하는 만큼 할인구매할 수 없기 때문에 둘째
        //날에도 회원가입을 하지 않습니다.
        ...
        public int solution17(string[] want, int[] number, string[] discount)
        {
            int answer = 0;
            for (int i = 0; i < discount.Length-9; i++)
            {
                string[] arr = discount.Skip(i).Take(10).ToArray();
                int cnt = 0;
                for (int j = 0; j < want.Length; j++)
                {
                    if (arr.Count(c => c == want[j]) != number[j]) break;
                    else cnt++;
                }
