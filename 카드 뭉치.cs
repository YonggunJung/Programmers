...
namespace ProgrammersLv1
{
    internal class 카드_뭉치
    {
        ...
        //입출력 예 #2
        //cards1에서 "i"를 사용하고 cards2에서 "want"와 "to"를 사용하여
        //"i want to"까지는 만들 수 있지만 "water"가 "drink"보다 먼저 사용되어야 하기 때문에
        //해당 문장을 완성시킬 수 없습니다. 따라서 "No"를 반환합니다.

        public string solution45(string[] cards1, string[] cards2, string[] goal)
        {
            string answer = "Yes";
            int c1 = 0, c2 = 0;
            for (int i = 0; i < goal.Length; i++)
            {
                if (c1 < cards1.Length && goal[i] == cards1[c1]) c1++;
                else if (c2 < cards2.Length &&goal[i] == cards2[c2]) c2++;
                else answer = "No";
            }
            return answer;
        }
