...
namespace ProgrammersLv1
{
    ...
    {
        //문제 설명
        //지나다니는 길을 'O', 장애물을 'X'로 나타낸
        //직사각형 격자 모양의 공원에서 로봇 강아지가 산책을 하려합니다.
        ...
        //입력된 명령대로라면 동쪽으로 2칸,
        //남쪽으로 2칸,
        //서쪽으로 1칸 이동해야하지만 남쪽으로 2칸 이동할 때
        //장애물이 있는 칸을 지나기 때문에 해당 명령을 제외한 명령들만 따릅니다.
        //결과적으로는[0, 0] -> [0, 2] -> [0, 1] 이 됩니다.

        //입출력 예 #3
        //처음 입력된 명령은 공원을 나가게 되고
        //두 번째로 입력된 명령 또한 장애물을 지나가게 되므로
        //두 입력은 제외한 세 번째 명령만 따르므로 결과는 다음과 같습니다.
        //[0, 1] -> [0, 0]

        public int[] solution65(string[] park, string[] routes)
        {
            int[] answer = new int[] { };
