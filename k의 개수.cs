...
namespace Programmers
{
    internal class k의_개수
    {
        ...
        public int solution154(int i, int j, int k)
        {
            int answer = 0;
            for (int x = i; x <= j; x++)
            {
        ...
        // 다른 사람 풀이
        // 쉽지 않네
        //Enumerable.Range(i, j - i + 1).Select(x => x.ToString().Where(y => y.Equals(k.ToString().First())).Count()).Sum();
