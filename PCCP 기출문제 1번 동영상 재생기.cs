...
{
    internal class PCCP_기출문제_1번_동영상_재생기
    {
        //문제 설명
        //당신은 동영상 재생기를 만들고 있습니다.
        //당신의 동영상 재생기는 10초 전으로 이동,
        //10초 후로 이동,
        ...
        //이때 사용자의 입력이 모두 끝난 후 동영상의 위치를 "mm:ss" 형식으로
        ...
        //비디오의 현재 위치 혹은 오프닝이 끝나는 시각이
        //동영상의 범위 밖인 경우는 주어지지 않습니다.
        ...
        //입출력 예 #2
        //시작 위치 0분 5초에서 10초 전으로 이동합니다.
        //현재 위치가 10초 미만이기 때문에 0분 0초로 이동합니다.
        ...
        //입출력 예 #3
        //시작 위치 4분 5초는 오프닝 구간이기 때문에
        //오프닝이 끝나는 위치인 4분 7초로 이동합니다.
        //4분 7초에서 10초 후로 이동하면 4분 17초입니다.
        //따라서 "04:17"을 return 하면 됩니다.

        public string solution67(string video_len, string pos, string op_start, string op_end, string[] commands)
        {
            string answer = "";
            string[] vt = video_len.Split(':');
            int at = int.Parse(vt[0]) * 60 + int.Parse(vt[1]);
            string[] poss = pos.Split(':');
            int po = int.Parse(poss[0]) * 60 + int.Parse(poss[1]);
            string[] opstt = op_start.Split(':');
            int ops = int.Parse(opstt[0]) * 60 + int.Parse(opstt[1]);
            string[] opend = op_end.Split(':');
            int ope = int.Parse(opend[0]) * 60 + int.Parse(opend[1]);

            if (po >= ops && po <= ope) po = ope;
